...
> 当前最新核心提交：`1ae2948 增强同步容错和链接补充`
> 最近一轮（未提交）改动：参见 `docs/CHANGELOG_DEV.md`，主要是 question.md 痛点修复、增量导入、OCR 骨架、doctor 增强。
...
已增加基于 `事项ID + 签阅人 + 签阅时间 + 签阅意见` 的唯一索引，重复导入同一签阅记录会被忽略。

后续如需支持“同一个人同一时间修改意见”的场景，可增加导入批次或记录更新时间字段。
