...
每次导入和解析大批文档时，可能重复解析。签阅记录重复导入的问题已通过唯一索引做了基础处理。
...
- [x] 输出清晰的成功/失败检查项。
- [x] 遇到失败给出下一步建议。
- [ ] 根据实机验证结果补充更精确的 AnythingLLM API 检查。
